(() => {
    ...
    document.body.addEventListener("click", (e => {
        if (!e.target.closest(".category")) {
            categories.forEach(((item, index) => {
                if (item.classList.contains("active")) switch (index) {
                  case 0:
    ...
    const script_form = document.querySelector(".authorization__form");
    ...
    async function formAction(e) {
        ...
        const removeError = () => {
            if (script_form.classList.contains("_error")) {
                script_form.classList.remove("_error");
                message.textContent = "";
            }
        };
        const clearForm = () => {
        ...
        if (response.ok) {
            ...
            if (responseResult.status === "ok") {
                ...
                script_form.classList.add("_error");
            }
        } else {
            console.log("Ошибка");
            removePreloader();
